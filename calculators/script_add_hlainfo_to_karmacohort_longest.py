...
if __name__ == '__main__':
    ...
    all_loci = ('hla_a', 'hla_b', 'hla_c', 'hla_dqb', 'hla_dr', 'hla_drb345', 'hla_dpa', 'hla_dpb', 'hla_dqa')
    ...
    df_karma_cohort.loc[
        ...
        df_karma_cohort.rec_hla_full.fillna('').str.len() >= df_karma_cohort.rec_hla_full_curr.fillna('').str.len(),
        df_karma_cohort.rec_hla_full,
        df_karma_cohort.rec_hla_full_curr
    )
    df_karma_cohort.loc[
        :, 'first_don_hla'
    ] = np.where(
        df_karma_cohort.first_don_hla_full.fillna('').str.len() >= df_karma_cohort.first_don_hla_full_curr.fillna('').str.len(),
        df_karma_cohort.first_don_hla_full,
        df_karma_cohort.first_don_hla_full_curr
    )

    hla_cols = (
        'rec_hla', 'first_don_hla', 'second_don_hla_full'
    )
    ...
    df_formatted_hlas.to_csv(
        'data/KARMA/karma_final_cohort/all_hlas_merged.csv',
        index=False,
        sep=','
    )
